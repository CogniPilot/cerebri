...
static void b3rb_fsm_init(struct context* ctx)
{
    ...
    zros_sub_init(&ctx->sub_input_ethernet, &ctx->node, &topic_input_ethernet, &ctx->input_ethernet, 10);
    zros_sub_init(&ctx->sub_cmd_vel_ethernet, &ctx->node, &topic_cmd_vel_ethernet, &ctx->cmd_vel_ethernet, 10);
...
static void fsm_compute_input(struct status_input* input, struct context* ctx)
{
    ...
    input->update_input_from_sbus = input->update_input_sbus && input->input_source_radio_control;
    ...
    input->update_topic_from_ethernet = input->update_cmd_vel_ethernet && input->topic_source_ethernet;
...
static void fsm_update(synapse_msgs_Status* status, const struct status_input* input)
{
    ...
    transition(
        ...
        // guards
        5,
        input->mode_unknown, "mode not set",
        input->mode_calibration, "mode calibration",
    ...
    transition(
        ...
        // guards
        1,
        input->input_status_loss, "no input data");
    ...
    transition(
        ...
        // guards
        1,
        input->topic_status_loss, "no topic data");
