...
static int sw_rx(uint8_t esc, uint8_t *byte, uint32_t timeout_us)
{
	...
	esc_input(esc);
	while (esc_read(esc)) {
		if ((k_uptime_get_32() - start) * 1000 > timeout_us) {
			return -ETIMEDOUT;
		}
	}
...
static int bl_read_data(uint8_t esc, uint8_t *data, uint16_t len)
{
	...
	LOG_INF("BL READ: %d bytes OK", len);
	return 0;
}
...
static int bl_write_data(uint8_t esc, const uint8_t *data, uint16_t len)
{
	...
	LOG_INF("BL WRITE: %d bytes PROG_FLASH OK", len);
	return 0;
}
			    ...
			    uint16_t len)
{
	...
	if (len == 0 && data == NULL) {
		...
		if (len > 0 && data) {
			...
			if (len > max_copy) {
				LOG_WRN("Truncating data copy: requested %zu bytes, only %zu fit in buffer", (size_t)len, max_copy);
			}
...
static void process_fourway(const uint8_t *buf, int len)
{
	...
	case FOURWAY_INTERFACE_SET_MODE: {
		...
		if (esc >= NUM_ESC_CHANNELS) {
			...
			current_esc = esc;
			LOG_INF("Select ESC %d", current_esc);
			fourway_send(cmd, ACK_OK, NULL, 0);
		}
		break;
	}
	case FOURWAY_DEVICE_INIT_FLASH: {
		uint8_t esc = (param_count >= 1) ? buf[5] : 0;
		LOG_INF(">>> DeviceInitFlash: esc=%d (4way_mode=%d)", esc, fourway_mode);
		...
		LOG_INF("Sending %d byte bootloader query to ESC%d", sizeof(boot_init), esc + 1);
		...
		LOG_INF("Query sent, waiting for response...");
		...
		for (int i = 0; i < 12 && got < 8; i++) {
			...
			if (sw_rx(esc, &byte, 50000) == 0) {
				resp[got++] = byte;
				LOG_INF("ESC%d[%d]: 0x%02X '%c'", esc + 1, got, byte,
					(byte >= 0x20 && byte < 0x7F) ? byte : '.');
			}
		}

		LOG_INF("ESC %d response: got %d bytes", esc + 1, got);

		if (got >= 8 && resp[0] == '4' && resp[1] == '7' && resp[2] == '1') {
			/* Valid BLHeli bootloader response - extract device info */
			/*
			 * Bootloader response bytes after "471":
			 *   resp[3] = 0x14 (input/version)
			 *   resp[4] = 0x1F (signature HIGH byte)
			 *   resp[5] = 0x06 (signature LOW byte)
			 *
			 * AM32 configurator expects params in this format:
			 *   params[0] = signature LOW byte
			 *   params[1] = signature HIGH byte
			 *   params[2] = input value
			 *   params[3] = interface mode
			 *
			 * Signature is built as: (params[1] << 8) | params[0]
			 * Known signatures: 0x1F06 (STM32F051), 0x3506 (ARM64K)
			 */
			...
			LOG_INF("ESC %d bootloader: sig=0x%04X input=%02X mode=%d", esc + 1, sig,
				info[2], mode);
	...
	case FOURWAY_DEVICE_RESET:
		LOG_INF("Reset ESC %d", current_esc);
...
handle_device_read: {
	...
	uint8_t data[256];
	LOG_INF("DeviceRead: %d bytes from 0x%04X (ESC %d)", rlen, addr, current_esc + 1);

	/* AM32 bootloader protocol:
	 * 1. Send SET_ADDRESS command to set flash address
	 * 2. Send READ command to read data
	 * Both commands require CRC-16 (polynomial 0xA001)
	 */
	int ret = bl_set_address(current_esc, addr);
	if (ret != 0) {
		LOG_WRN("DeviceRead: SET_ADDRESS failed");
	...
	if (ret != 0) {
		LOG_WRN("DeviceRead: READ failed");
		fourway_send_ex(cmd, addr, ACK_GENERAL_ERROR, NULL, 0);
		break;
	}

	LOG_INF("DeviceRead: got %d bytes", rlen);
	LOG_HEXDUMP_INF(data, (rlen > 32) ? 32 : rlen, "Data:");
...
handle_device_write: {
	...
	if (addr >= 0x7C00 && addr < 0x8000 && !eeprom_erased[current_esc]) {
		LOG_INF("DeviceWrite: Auto-erasing EEPROM page at 0x7C00 (first write, ESC %d)",
			current_esc + 1);
		...
		LOG_INF("DeviceWrite: Auto-erase OK (won't erase ESC %d again)", current_esc + 1);
	...
	LOG_INF("DeviceWrite: OK -> sending ACK");
	fourway_send_ex(cmd, addr, ACK_OK, NULL, 0);
	break;
}
	case FOURWAY_DEVICE_PAGE_ERASE: {
		LOG_INF("PageErase: 0x%04X (ESC %d)", addr, current_esc + 1);
...
static void process_rx(void)
{
	...
	/* Debug: show any received data in 4-way mode */
	if (fourway_mode && local_len > 0) {
		LOG_INF("RX buf: %d bytes, first=%02X", local_len, rx_buf[0]);
	}
