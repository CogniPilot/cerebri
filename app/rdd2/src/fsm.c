				  ...
				  const struct context *ctx)
{
	...
	double bat_max = CONFIG_CEREBRI_RDD2_BATTERY_NCELLS *
			 CONFIG_CEREBRI_RDD2_BATTERY_CELL_MAX_MILLIVOLT / 1000.0;
	double bat_min = CONFIG_CEREBRI_RDD2_BATTERY_NCELLS *
			 CONFIG_CEREBRI_RDD2_BATTERY_CELL_MIN_MILLIVOLT / 1000.0;
...
static int start(struct context *ctx)
{
	k_tid_t tid = k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				      rdd2_fsm_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "rdd2_fsm");
	k_thread_start(tid);
	return 0;
}

static int rdd2_fsm_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_rdd2_fsm, rdd2_fsm_cmd_handler, (start, &g_ctx, "start"),
