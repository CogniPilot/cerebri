...
CEREBRI_NODE_LOG_INIT(rdd2_lighting, LOG_LEVEL_WRN);
...
static int start(struct context *ctx)
{
	k_tid_t tid =
		k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				rdd2_lighting_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "rdd2_lighting");
	k_thread_start(tid);
	return 0;
}

static int rdd2_lighting_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_rdd2_lighting, rdd2_lighting_cmd_handler, (start, &g_ctx, "start"),
