...
def derive_se2():
    ...
    se2_error = ca.Function('se2_error',
            [p, r], [SE2.log(SE2.product(SE2.inv(p), r))],
            ['p', 'r'], ['error'])
...
def code_gen():
    ...
    gen_dir = script_dir / "src" / "casadi"
    gen_dir.mkdir(parents=True, exist_ok=True)

    os.chdir(gen_dir)
    gen_rover = ca.CodeGenerator('rover', code_opts)

    for key, val in bezier6.items():
        gen_rover.add(val)

    for key, val in rover.items():
        gen_rover.add(val)

    for key, val in diff_correction.items():
        gen_rover.add(val)

    gen_rover.generate()
    os.chdir(script_dir)
    print(f'generation completed: {gen_dir}')
