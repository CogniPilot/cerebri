...
## Summary
`cubs2` follows a CEP-0002 platform layout, and the active `cubs2` Zephyr app
lives under `cubs2/` with a platform-local manifest, specs, and build wiring.

## Specification

**REQUIRED:**
- The `cubs2` Zephyr application lives under `cubs2/`, not at the repository
  root.
- The `cubs2` platform manifest lives in `cubs2/west.yml`.
- The repository root does not carry a compatibility `west.yml`.
- Platform-local generated output ignore rules live in `cubs2/.gitignore`, while
