...
static int cmd_motor_status(const struct shell *sh, size_t argc, char **argv)
{
	...
	active = cubs2_motor_test_get(&test_motors);
	raw_active = cubs2_motor_raw_test_get(&test_raw);
...
static int cmd_motor_stop(const struct shell *sh, size_t argc, char **argv)
{
	...
	cubs2_motor_test_clear();
	cubs2_motor_raw_test_clear();
	if (cubs2_motor_output_ready()) {
		cubs2_motor_output_write_all(&(synapse_topic_MotorValues4f_t){0}, false, false);
	}
...
static int cmd_motor_spin(const struct shell *sh, size_t argc, char **argv)
{
	...
	cubs2_motor_test_set((size_t)index, value);
...
static int cmd_motor_raw(const struct shell *sh, size_t argc, char **argv)
{
	...
	cubs2_motor_test_clear();
	cubs2_motor_raw_test_set((size_t)index, (uint16_t)value);
...
static int cmd_motor_raw_all(const struct shell *sh, size_t argc, char **argv)
{
	...
	cubs2_motor_test_clear();
	cubs2_motor_raw_test_set_all((uint16_t)value);
