...
static synapse_topic_FlightSnapshot_table_t flight_state_root(
	const uint8_t *buf, size_t buf_size)
{
	...
	if (buf_size != CUBS2_TOPIC_FB_FLIGHT_STATE_SIZE) {
		return NULL;
	}
...
static synapse_topic_MotorOutput_table_t motor_output_root(const uint8_t *buf, size_t buf_size)
{
	...
	if (buf_size != CUBS2_TOPIC_FB_MOTOR_OUTPUT_SIZE) {
		return NULL;
	}
	...
	const synapse_topic_RateTriplet_t *rate_cmd)
{
	...
	length = pack_fixed_table(buf, buf_size, CUBS2_TOPIC_FB_FLIGHT_STATE_SIZE,
	...
	memcpy(table + FLIGHT_FIELD_ATTITUDE_DESIRED, attitude_desired,
	...
	synapse_topic_RateTriplet_t *rate_desired, synapse_topic_RateTriplet_t *rate_cmd)
{
	...
	memcpy(attitude_desired,
	       synapse_topic_FlightSnapshot_attitude_desired(flight_state),
	       sizeof(*attitude_desired));
	...
	const synapse_topic_MotorRaw4u16_t *raw, bool armed, bool test_mode)
{
	...
	length = pack_fixed_table(buf, buf_size, CUBS2_TOPIC_FB_MOTOR_OUTPUT_SIZE,
