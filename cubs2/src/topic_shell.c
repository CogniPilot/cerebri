...
struct flight_state_store {
	uint8_t slots[2][CUBS2_TOPIC_FB_FLIGHT_STATE_SIZE];
	uint16_t lengths[2];
	atomic_t generation;
};

struct motor_output_store {
	uint8_t slots[2][CUBS2_TOPIC_FB_MOTOR_OUTPUT_SIZE];
	uint16_t lengths[2];
	atomic_t generation;
};
	...
	const synapse_topic_RateTriplet_t *rate_cmd)
{
	...
	length = cubs2_topic_fb_pack_flight_state(
	...
	synapse_topic_RateTriplet_t *rate_desired, synapse_topic_RateTriplet_t *rate_cmd)
{
	...
	return cubs2_topic_fb_unpack_flight_state(buf, len, gyro, accel, rc, status, attitude,
						     attitude_desired, rate_desired, rate_cmd);
}
				       ...
				       bool test_mode)
{
	...
	slot = next_generation & 1U;
	length = cubs2_topic_fb_pack_motor_output(
					  ...
					  bool *test_mode)
{
	...
	return cubs2_topic_fb_unpack_motor_output(buf, len, motors, raw, armed, test_mode);
}
...
void cubs2_topic_rc_published(void)
{
	topic_seq_bump(TOPIC_RC);
}

void cubs2_topic_motor_output_publish(const synapse_topic_MotorValues4f_t *motors,
					 const synapse_topic_MotorRaw4u16_t *raw, bool armed,
					 bool test_mode)
{
...
bool cubs2_topic_motor_output_get(synapse_topic_MotorValues4f_t *motors,
				     synapse_topic_MotorRaw4u16_t *raw, bool *armed,
				     bool *test_mode)
{
...
bool cubs2_topic_motor_output_copy_blob(uint8_t *buf, size_t buf_size, size_t *len)
{
	return motor_output_store_copy_latest_blob(buf, buf_size, len);
}

uint32_t cubs2_topic_motor_output_generation(void)
{
	return (uint32_t)atomic_get(&g_motor_output_store.generation);
}

void cubs2_topic_flight_state_publish(
	const synapse_topic_Vec3f_t *gyro, const synapse_topic_Vec3f_t *accel,
	const synapse_topic_RcChannels16_t *rc, const synapse_topic_ControlStatus_t *status,
	const synapse_topic_AttitudeEuler_t *attitude,
	const synapse_topic_AttitudeEuler_t *attitude_desired,
...
bool cubs2_topic_flight_state_get(
	synapse_topic_Vec3f_t *gyro, synapse_topic_Vec3f_t *accel,
	synapse_topic_RcChannels16_t *rc, synapse_topic_ControlStatus_t *status,
	synapse_topic_AttitudeEuler_t *attitude,
	synapse_topic_AttitudeEuler_t *attitude_desired,
...
bool cubs2_topic_flight_state_copy_blob(uint8_t *buf, size_t buf_size, size_t *len)
{
	return flight_state_store_copy_latest_blob(buf, buf_size, len);
}

uint32_t cubs2_topic_flight_state_generation(void)
{
				      ...
				      synapse_topic_RateTriplet_t *rate_cmd)
{
	...
	(void)cubs2_topic_flight_state_get(gyro, accel, rc, status, attitude,
					     attitude_desired, rate_desired, rate_cmd);
}
				      ...
				      bool *test_mode)
{
	...
	*armed = false;
	*test_mode = false;
	(void)cubs2_topic_motor_output_get(motors, raw, armed, test_mode);
}
