...
LOG_MODULE_REGISTER(cubs2_sitl_udp, LOG_LEVEL_INF);

struct sitl_input_store {
	uint8_t slots[2][CUBS2_SITL_INPUT_MAX_SIZE];
	uint16_t lengths[2];
	atomic_t generation;
};
...
static int sitl_destination_init(struct sockaddr_in *addr, uint16_t port)
{
	...
	if (inet_pton(AF_INET, CONFIG_CUBS2_SITL_HOST, &addr->sin_addr) != 1) {
		return -EINVAL;
	}
				 ...
				 bool rc_valid)
{
	...
	const int32_t *channels = cubs2_topic_rc_channels_data_const(rc);
	...
	(void)input_report(rc_dev, INPUT_EV_MSC, CUBS2_RC_INPUT_EVENT_LINK_QUALITY,
			   rc_link_quality, false, K_FOREVER);
	(void)input_report(rc_dev, INPUT_EV_MSC, CUBS2_RC_INPUT_EVENT_VALID,
...
static void sitl_send_flight_state_if_updated(void)
{
	...
	uint32_t generation = cubs2_topic_flight_state_generation();
	...
	if (!cubs2_topic_flight_state_copy_blob(buf, sizeof(buf), &len)) {
		return;
	}
...
{
	static uint32_t last_generation;
	uint32_t generation = cubs2_topic_motor_output_generation();
	...
	if (!cubs2_topic_motor_output_copy_blob(buf, sizeof(buf), &len)) {
		return;
	}
...
static int cubs2_sitl_init(void)
{
	int rc;

	atomic_set(&g_sitl_input_store.generation, 0);

	rc = sitl_socket_init(&g_sitl_rx_sock, CONFIG_CUBS2_SITL_RX_PORT);
	if (rc != 0) {
	...
	rc = sitl_destination_init(&g_sitl_tx_flight_addr,
				       CONFIG_CUBS2_SITL_TX_FLIGHT_SNAPSHOT_PORT);
	if (rc != 0) {
	...
	rc = sitl_destination_init(&g_sitl_tx_motor_addr,
				       CONFIG_CUBS2_SITL_TX_MOTOR_OUTPUT_PORT);
	if (rc != 0) {
	...
	k_thread_create(&g_sitl_thread, g_sitl_thread_stack,
			...
			NULL, NULL, NULL, CONFIG_CUBS2_SITL_THREAD_PRIORITY, 0, K_NO_WAIT);
	k_thread_name_set(&g_sitl_thread, "cubs2_sitl");

	LOG_INF("sitl udp rx=%d tx_flight=%d tx_motor=%d host=%s",
		CONFIG_CUBS2_SITL_RX_PORT, CONFIG_CUBS2_SITL_TX_FLIGHT_SNAPSHOT_PORT,
		CONFIG_CUBS2_SITL_TX_MOTOR_OUTPUT_PORT, CONFIG_CUBS2_SITL_HOST);

	return 0;
}

SYS_INIT(cubs2_sitl_init, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);
