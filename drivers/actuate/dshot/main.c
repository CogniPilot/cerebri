...
static int actuate_dshot_init(struct context *ctx) {
  ...
  if (ctx->num_actuators != nxp_flexio_dshot_channel_count(ctx->dev)) {
    LOG_ERR("dshot actuator config mismatch! %i %i", ctx->num_actuators, nxp_flexio_dshot_channel_count(ctx->dev));
  }
  zros_node_init(&ctx->node, "actuate_dshot");
  zros_sub_init(&ctx->sub_actuators, &ctx->node, &topic_actuators,
                &ctx->actuators, 1000);
...
static void dshot_spin_dir(const struct shell *sh, struct context *ctx,
                           int motor, bool reverse) {
...
static int start(struct context *ctx) {
  ...
  k_tid_t tid = k_thread_create(&ctx->thread_data, ctx->stack_area,
                                ctx->stack_size, actuate_dshot_run, ctx, NULL,
...
static int actuate_dshot_cmd_handler(const struct shell *sh, size_t argc,
                                     char **argv, void *data) {
  ARG_UNUSED(argc);
...
#define DSHOT_ACTUATOR_SHELL(inst)                                             \
  static int actuate_dshot_cmd_handler_##inst(                                 \
      const struct shell *sh, size_t argc, char **argv, void *data) {          \
...
#define DSHOT_ACTUATOR_DEFINE(node_id)                                         \
  {                                                                            \
      ...
      .scale =                                                                 \
          ((double)DT_PROP(node_id, scale)) / DT_PROP(node_id, scale_div),     \
...
#define DSHOT_ACTUATORS_DEFINE(inst)                                           \
  static const actuator_dshot_t g_actuator_dshots_##inst[] = {                 \
      DT_FOREACH_CHILD(DT_INST(inst, cerebri_dshot_actuators),                 \
                       DSHOT_ACTUATOR_DEFINE)};                                \
  ...
  DEVICE_DT_INST_DEFINE(inst, actuate_dshot_device_init, NULL, &data_##inst,   \
                        NULL, POST_KERNEL,                                     \
                        CONFIG_DSHOT_ACTUATORS_INIT_PRIORITY, NULL);
