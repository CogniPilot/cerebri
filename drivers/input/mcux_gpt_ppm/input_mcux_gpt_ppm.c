    ...
    void* dummy3)
{
    ...
    while (true) {
        ...
        for (i = 0; i < config->num_channels; i++) {
            ...
            if (data->ppm_buffer[channel] > (ppm_last_reported[channel] + PPM_FILTER) || data->ppm_buffer[channel] < (ppm_last_reported[channel] - PPM_FILTER)) {
                ...
                case INPUT_EV_KEY:
                    if (data->ppm_buffer[channel] > PPM_CHANNEL_VALUE_ONE) {
                        input_report_key(
                            ...
                            false, K_FOREVER);
                        ppm_last_reported[channel] = data->ppm_buffer[channel];
                    } else if (data->ppm_buffer[channel] < PPM_CHANNEL_VALUE_ZERO) {
                        input_report_key(
...
int input_mcux_gpt_ppm_init(const struct device* dev)
{
    ...
    for (i = 0; i < config->num_channels; i++) {
        if (config->channel_info[i].channel == 0 || config->channel_info[i].channel > PPM_MAX_CHANNELS) {
