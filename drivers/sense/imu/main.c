...
void imu_publish(context_t *ctx)
{
	...
	ctx->imu.linear_acceleration.x = (ctx->accel_raw[0] - ctx->accel_bias[0]) / ctx->accel_scale;
	ctx->imu.linear_acceleration.y = (ctx->accel_raw[1] - ctx->accel_bias[1]) / ctx->accel_scale;
	ctx->imu.linear_acceleration.z = (ctx->accel_raw[2] - ctx->accel_bias[2]) / ctx->accel_scale;
