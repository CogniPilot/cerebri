...
static int sitl_imu_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (!cerebri_sitl_udp_latest_input_get(buf, sizeof(buf), &len, &generation)) {
		data->valid = false;
		return -ENODATA;
	}
	...
	if (!cerebri_sitl_fb_unpack_input(buf, len, &data->gyro, &data->accel, NULL, NULL, NULL,
					   &imu_valid)) {
		data->valid = false;
...
#define CEREBRI_SITL_IMU_INIT(inst)                                                              \
...
DT_INST_FOREACH_STATUS_OKAY(CEREBRI_SITL_IMU_INIT)
