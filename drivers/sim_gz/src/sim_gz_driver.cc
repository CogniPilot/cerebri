...
static const double actuator_scalar = 1000;

static const std::string actuators_topic = "/x500/command/motor_speed";
...
static gz::msgs::Actuators actuators{};
static gz::msgs::Twist twist{};
static gz::msgs::Joy joy{};
static std::weak_ptr<gz::transport::Node::Publisher> pub_actuators_ptr{};
...
void send_control(sim_time_t time, const msg_actuators_t * msg) {
    ...
    // send actuators
    actuators.set_velocity(0, actuator_scalar*msg->ch0);
    actuators.set_velocity(1, actuator_scalar*msg->ch1);
    actuators.set_velocity(2, actuator_scalar*msg->ch2);
    actuators.set_velocity(3, actuator_scalar*msg->ch3);
    if (!pub_actuators_ptr.lock().get()->Publish(actuators)) {
        std::cerr << "Error publishing topic [" << actuators_topic << "]" << std::endl;
    }
}
...
void thread_sim_entry_point(void)
{
    ...
    actuators.add_velocity(0);
    actuators.add_velocity(0);
    actuators.add_velocity(0);
    actuators.add_velocity(0);
    actuators.set_allocated_header(&header);

    // actuator pub
    auto pub_actuators = std::make_shared<gz::transport::Node::Publisher>(
        node.Advertise<gz::msgs::Actuators>(actuators_topic));
    if (!pub_actuators)
    {
        std::cerr << "Error advertising topic [" << actuators_topic << "]" << std::endl;
        return;
    }
    pub_actuators_ptr = pub_actuators;
