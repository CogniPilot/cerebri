...
#define DT_DRV_COMPAT cognipilot_sitl_udp
...
LOG_MODULE_REGISTER(cerebri2_sitl_udp, LOG_LEVEL_INF);

BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT) == 1,
	     "cerebri2 native_sim expects exactly one SITL UDP coordinator");

struct sitl_input_store {
	uint8_t slots[2][CEREBRI2_SITL_INPUT_MAX_SIZE];
	uint16_t lengths[2];
	atomic_t generation;
};
...
static K_THREAD_STACK_DEFINE(g_sitl_thread_stack, CONFIG_CEREBRI2_SITL_THREAD_STACK_SIZE);
...
static int sitl_destination_init(struct sockaddr_in *addr, uint16_t port)
{
	...
	if (inet_pton(AF_INET, CONFIG_CEREBRI2_SITL_HOST, &addr->sin_addr) != 1) {
		return -EINVAL;
	}
...
static void sitl_report_rc_input(const struct sitl_input_msg *input)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(input->rc.us); i++) {
		(void)input_report_abs(rc_dev, (uint16_t)(i + 1U), input->rc.us[i], false,
				       K_FOREVER);
	}

	(void)input_report(rc_dev, INPUT_EV_MSC, CEREBRI2_RC_INPUT_EVENT_LINK_QUALITY,
			   input->rc_link_quality, false, K_FOREVER);
	(void)input_report(rc_dev, INPUT_EV_MSC, CEREBRI2_RC_INPUT_EVENT_VALID,
			   input->rc_valid ? 1 : 0, true, K_FOREVER);
}
...
static void sitl_rx_drain(void)
{
	...
	while (true) {
		...
		if (!cerebri2_sitl_fb_unpack_input(buf, (size_t)len, &input)) {
			source_addr_len = sizeof(source_addr);
			continue;
		}

		sitl_input_store_publish(buf, (size_t)len);
		sitl_report_rc_input(&input);
		source_addr_len = sizeof(source_addr);
	}
}

static void sitl_send_flight_snapshot_if_updated(void)
{
	static uint32_t last_generation;
	uint32_t generation = cerebri2_topic_flight_snapshot_generation();
	...
	snapshot = cerebri2_topic_flight_snapshot_get();
	len = cerebri2_topic_fb_pack_flight_snapshot(buf, sizeof(buf), &snapshot);
	if (len == 0U) {
		return;
	}
...
{
	static uint32_t last_generation;
	uint32_t generation = cerebri2_topic_motor_output_generation();
	...
	output = cerebri2_topic_motor_output_snapshot();
	len = cerebri2_topic_fb_pack_motor_output(buf, sizeof(buf), &output);
	if (len == 0U) {
		return;
	}
...
static void sitl_transport_thread(void *arg0, void *arg1, void *arg2)
{
	...
	while (true) {
		sitl_rx_drain();
		sitl_send_flight_snapshot_if_updated();
		sitl_send_motor_output_if_updated();
		k_sleep(K_MSEC(1));
	}
}

static int sitl_udp_init(const struct device *dev)
{
	int rc;
	...
	rc = sitl_socket_init(&g_sitl_rx_sock, CONFIG_CEREBRI2_SITL_RX_PORT);
	if (rc != 0) {
	...
	rc = sitl_destination_init(&g_sitl_tx_flight_addr,
				       CONFIG_CEREBRI2_SITL_TX_FLIGHT_SNAPSHOT_PORT);
	if (rc != 0) {
	...
	rc = sitl_destination_init(&g_sitl_tx_motor_addr,
				       CONFIG_CEREBRI2_SITL_TX_MOTOR_OUTPUT_PORT);
	if (rc != 0) {
	...
	k_thread_create(&g_sitl_thread, g_sitl_thread_stack,
			...
			NULL, NULL, NULL, CONFIG_CEREBRI2_SITL_THREAD_PRIORITY, 0, K_NO_WAIT);
	k_thread_name_set(&g_sitl_thread, "cerebri2_sitl");

	LOG_INF("sitl udp rx=%d tx_flight=%d tx_motor=%d host=%s",
		CONFIG_CEREBRI2_SITL_RX_PORT, CONFIG_CEREBRI2_SITL_TX_FLIGHT_SNAPSHOT_PORT,
		CONFIG_CEREBRI2_SITL_TX_MOTOR_OUTPUT_PORT, CONFIG_CEREBRI2_SITL_HOST);

	return 0;
}

DEVICE_DT_INST_DEFINE(0, sitl_udp_init, NULL, NULL, NULL, POST_KERNEL,
