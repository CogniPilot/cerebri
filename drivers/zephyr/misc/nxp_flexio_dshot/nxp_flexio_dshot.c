...
static const uint32_t gcr_decode[32] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
...
static void nxp_flexio_dshot_output(const struct device* dev,
    uint32_t channel)
{
    ...
    FLEXIO_SetShifterConfig(flexio_base, child->res.shifter_index[channel],
        &shifterConfig);
    ...
    timerConfig.triggerSelect = FLEXIO_TIMER_TRIGGER_SEL_SHIFTnSTAT(child->res.shifter_index[channel]);
    ...
    FLEXIO_SetTimerConfig(flexio_base, child->res.timer_index[channel],
        &timerConfig);
}

static void nxp_flexio_bdshot_input(const struct device* dev,
    uint32_t channel)
{
    const struct nxp_flexio_dshot_config* config = dev->config;
    struct nxp_flexio_dshot_data* data = dev->data;
    ...
    FLEXIO_ClearShifterStatusFlags(flexio_base,
        1 << child->res.shifter_index[channel]);
...
static int nxp_flexio_dshot_init(const struct device* dev)
{
    ...
    data->bdshot_tcmp = 0x2900 | (((data->flexio_clk / (dshot_pwm_freq * 5 / 4) / 2) - 3) & 0xFF);
...
void nxp_flexio_dshot_trigger(const struct device* dev)
{
    ...
    for (uint8_t channel = 0; (channel < config->channel->dshot_channel_count);
         channel++) {
        ...
        if (data->bdshot_recv_mask & shifter_flag) {
            ...
            if (value & 0x1) {
                ...
                if ((csum_data & 0xFU) != 0xFU) {
                    ...
                    config->channel->dshot_info[channel].last_no_response_cnt = config->channel->dshot_info[channel].no_response_cnt;
                }
...
static int nxp_flexio_dshot_isr(void* user_data)
{
    ...
    for (channel = 0; flags && channel < config->channel->dshot_channel_count;
         channel++) {
        shifter_flag = 1 << child->res.shifter_index[channel];
        ...
        if (flags & shifter_flag) {
            ...
            if (dshot_info->state == DSHOT_START) {
                ...
                flexio_base->SHIFTBUF[child->res.shifter_index[channel]] = dshot_info->irq_data;
    ...
    for (channel = 0; (flags & data->dshot_mask);
        ...
        if (flags & timer_flag) {
            ...
            } else if (!dshot_info->bdshot && dshot_info->state == DSHOT_12BIT_TRANSFERRED) {
...
static int nxp_flexio_dshot_sample_fetch(const struct device* dev,
...
static int nxp_flexio_dshot_channel_get(const struct device* dev,
    enum sensor_channel chan,
...
#define FLEXIO_DSHOT_FLEXIO_INDEX_INIT(n)                                    \
    static uint8_t                                                           \
        flexio_dshot_##n##_timer_index[ARRAY_SIZE(flexio_dshot_##n##_init)]; \
    static uint8_t                                                           \
        flexio_dshot_##n##_shifter_index[ARRAY_SIZE(flexio_dshot_##n##_init)];
...
#define NXP_FLEXIO_DSHOT_INIT(n)                                                         \
    ...
    SENSOR_DEVICE_DT_INST_DEFINE(                                                        \
        n, &nxp_flexio_dshot_init, NULL, &nxp_flexio_dshot_data_##n,                     \
        &nxp_flexio_dshot_config_##n, POST_KERNEL,                                       \
        CONFIG_NXP_FLEXIO_DSHOT_INIT_PRIORITY, &nxp_flexio_dshot_api_funcs);
