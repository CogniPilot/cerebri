...
LOG_MODULE_REGISTER(control_diffdrive, CONFIG_CEREBRI_CONTROL_DIFFDRIVE_LOG_LEVEL);
...
{
    syn_node_init(&ctx->node, "control_diffdrive_fsm");
    ...
    synapse_msgs_Fsm* fsm)
{
    ...
    bool safe = false;
#endif
    bool battery_critical = battery_state->voltage < CONFIG_CEREBRI_CONTROL_DIFFDRIVE_BATTERY_MIN_MILLIVOLT / 1000.0;
...
static void control_diffdrive_fsm_run(context* ctx)
{
...
static void listener_control_diffdrive_fsm_callback(const struct zbus_channel* chan)
{
    syn_node_listen(&g_ctx.node, chan, K_MSEC(100));
}

ZBUS_LISTENER_DEFINE(listener_control_diffdrive_fsm, listener_control_diffdrive_fsm_callback);
ZBUS_CHAN_ADD_OBS(chan_battery_state, listener_control_diffdrive_fsm, 1);
ZBUS_CHAN_ADD_OBS(chan_joy, listener_control_diffdrive_fsm, 1);
ZBUS_CHAN_ADD_OBS(chan_safety, listener_control_diffdrive_fsm, 1);
/* vi: ts=4 sw=4 et */
