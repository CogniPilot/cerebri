...
void imu_calibrate(context_t * ctx)
{
    ...
    for (int j = 0; j < CONFIG_CEREBRI_SENSE_IMU_ACCEL_COUNT; j++) {
        ...
        ctx->accel_scale[j] = sqrt(
            accel_mean[j][0]*accel_mean[j][0] + 
            accel_mean[j][1]*accel_mean[j][1] + 
            accel_mean[j][2]*accel_mean[j][2])/g_accel;
        LOG_INF("accel %d", j);
...
void imu_publish(context_t * ctx)
{
    ...
    ctx->imu.linear_acceleration.x =
        (ctx->accel_raw[accel_select][0] - ctx->accel_bias[accel_select][0])/ctx->accel_scale[accel_select];
    ctx->imu.linear_acceleration.y =
        (ctx->accel_raw[accel_select][1] - ctx->accel_bias[accel_select][1])/ctx->accel_scale[accel_select];
    ctx->imu.linear_acceleration.z =
        (ctx->accel_raw[accel_select][2] - ctx->accel_bias[accel_select][2])/ctx->accel_scale[accel_select];
