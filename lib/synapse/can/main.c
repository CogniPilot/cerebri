...
const struct can_filter zfilter = {
...
static void tx(int* can_fd)
{
    ...
    while (1) {
        ret = zsock_send(fd, &sframe, sizeof(sframe), 0);
        if (ret < 0) {
...
static int create_socket(const struct socketcan_filter* sfilter)
{
    ...
    fd = zsock_socket(AF_CAN, SOCK_RAW, CAN_RAW);
    if (fd < 0) {
    ...
    ret = zsock_bind(fd, (struct sockaddr*)&can_addr, sizeof(can_addr));
    if (ret < 0) {
    ...
    (void)zsock_setsockopt(fd, SOL_CAN_RAW, CAN_RAW_FILTER, sfilter,
        sizeof(*sfilter));
    ...
    const struct socketcan_filter* sfilter)
{
    ...
    while (1) {
        ...
        ret = zsock_recvfrom(fd, &sframe, sizeof(struct socketcan_frame),
...
static int setup_socket(void)
{
    ...
    ret = zsock_setsockopt(fd, SOL_CAN_RAW, CAN_RAW_FILTER, &sfilter,
        sizeof(sfilter));
    if (ret < 0) {
        ret = -errno;
...
void synapse_can_entry_point(void* p1, void* p2, void* p3)
{
    ...
    ret = ret = can_set_mode(dev, CAN_MODE_FD);
    ;
    if (ret != 0) {
...
K_THREAD_DEFINE(synapse_can, MY_STACK_SIZE,
    synapse_can_entry_point, NULL, NULL, NULL,
    MY_PRIORITY, 0, 0);
