...
## Summary
Minimal latency is a primary system driver, so the flight stack keeps one application hot-path thread, paces flight hardware from the IMU FIFO watermark at 800 Hz, and runs estimator prediction, mode selection, control, mixing, and motor output in that same thread with no extra handoff.
...
- The control loop consumes the latest decoded IMU sample, estimator prediction, mode selection, controller execution, mixing, and DSHOT trigger in that one thread.
...
- If attitude correction exists, it must stay out of the 800 Hz hot path and must not gate motor output.
