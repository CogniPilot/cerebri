...
static int cmd_fc_status(const struct shell *sh, size_t argc, char **argv)
{
	...
	fc_latest_state(&gyro, &accel, &rc, &status, &attitude, &attitude_desired,
			&rate_desired, &rate_cmd, &main_loop_latency_us);
	rc_age_ms = status.rc_valid ? (now_ms - status.rc_stamp_ms) : -1;

	shell_print(sh,
		    "mode=%s armed=%d imu_ok=%d rc_valid=%d rc_stale=%d arm_switch=%d rc_age_ms=%lld lq=%u throttle_us=%ld imu_to_motor_us=%lu",
	...
	shell_print(sh,
		    ...
		    (double)rate_cmd.yaw);
	shell_print(sh,
		    "attitude roll=%0.3f pitch=%0.3f yaw=%0.3f attitude_desired roll=%0.3f pitch=%0.3f yaw=%0.3f",
