...
BUILD_ASSERT(sizeof(synapse_topic_ControlStatus_t) == 24U);
BUILD_ASSERT(sizeof(synapse_topic_AttitudeEuler_t) == 12U);
BUILD_ASSERT(sizeof(synapse_topic_RateTriplet_t) == 12U);
BUILD_ASSERT(sizeof(synapse_topic_MotorValues4f_t) == 16U);
BUILD_ASSERT(sizeof(synapse_topic_MotorRaw4u16_t) == 8U);
...
static size_t pack_fixed_table(uint8_t *buf, size_t buf_size, size_t total_size,
			       uint16_t vtable_size, uint16_t object_size,
...
static synapse_topic_FlightSnapshot_table_t flight_state_root(const uint8_t *buf, size_t buf_size)
{
	...
	if (buf_size != RDD2_TOPIC_FB_FLIGHT_STATE_SIZE) {
		return NULL;
	}
	...
	if (get_le16(vtable + 0) != FLIGHT_VTABLE_SIZE ||
	    ...
	    get_le16(vtable + 6) != FLIGHT_FIELD_ACCEL || get_le16(vtable + 8) != FLIGHT_FIELD_RC ||
	...
	return synapse_topic_FlightSnapshot_as_root(buf);
}

static synapse_topic_MotorOutput_table_t motor_output_root(const uint8_t *buf, size_t buf_size)
{
	const uint8_t *vtable;
	const uint8_t *table;
	uint32_t root_offset;
	...
	if (buf_size != RDD2_TOPIC_FB_MOTOR_OUTPUT_SIZE) {
		return NULL;
	}
	...
	if (get_le16(vtable + 0) != MOTOR_VTABLE_SIZE ||
	    ...
	    get_le16(vtable + 4) != MOTOR_FIELD_MOTORS || get_le16(vtable + 6) != MOTOR_FIELD_RAW ||
	    get_le16(vtable + 8) != MOTOR_FIELD_ARMED ||
	    get_le16(vtable + 10) != MOTOR_FIELD_TEST_MODE) {
		return NULL;
	}

	return synapse_topic_MotorOutput_as_root(buf);
}
...
size_t rdd2_topic_fb_pack_flight_state(
	...
	const synapse_topic_ControlStatus_t *status, const synapse_topic_AttitudeEuler_t *attitude,
	...
	const synapse_topic_RateTriplet_t *rate_cmd, uint32_t main_loop_latency_us)
{
	static const uint16_t field_offsets[] = {
		FLIGHT_FIELD_GYRO,         FLIGHT_FIELD_ACCEL,    FLIGHT_FIELD_RC,
		FLIGHT_FIELD_STATUS,       FLIGHT_FIELD_ATTITUDE, FLIGHT_FIELD_ATTITUDE_DESIRED,
		FLIGHT_FIELD_RATE_DESIRED, FLIGHT_FIELD_RATE_CMD, FLIGHT_FIELD_MAIN_LOOP_LATENCY_US,
	};
	uint8_t *table;
	size_t length;

	if (gyro == NULL || accel == NULL || rc == NULL || status == NULL || attitude == NULL ||
	    attitude_desired == NULL || rate_desired == NULL || rate_cmd == NULL) {
		return 0U;
	}

	length =
		pack_fixed_table(buf, buf_size, RDD2_TOPIC_FB_FLIGHT_STATE_SIZE, FLIGHT_VTABLE_SIZE,
				 FLIGHT_OBJECT_SIZE, field_offsets, ARRAY_SIZE(field_offsets));
	if (length == 0U) {
		return 0U;
	}
	...
	memcpy(table + FLIGHT_FIELD_ATTITUDE, attitude, sizeof(*attitude));
	memcpy(table + FLIGHT_FIELD_ATTITUDE_DESIRED, attitude_desired, sizeof(*attitude_desired));
...
bool rdd2_topic_fb_unpack_flight_state(
	...
	synapse_topic_AttitudeEuler_t *attitude_desired, synapse_topic_RateTriplet_t *rate_desired,
	synapse_topic_RateTriplet_t *rate_cmd, uint32_t *main_loop_latency_us)
{
	...
	if (flight_state == NULL || gyro == NULL || accel == NULL || rc == NULL || status == NULL ||
	    attitude == NULL || attitude_desired == NULL || rate_desired == NULL ||
	    rate_cmd == NULL || main_loop_latency_us == NULL) {
		return false;
	}

	memcpy(gyro, synapse_topic_FlightSnapshot_gyro(flight_state), sizeof(*gyro));
	memcpy(accel, synapse_topic_FlightSnapshot_accel(flight_state), sizeof(*accel));
	memcpy(rc, synapse_topic_FlightSnapshot_rc(flight_state), sizeof(*rc));
	memcpy(status, synapse_topic_FlightSnapshot_status(flight_state), sizeof(*status));
	memcpy(attitude, synapse_topic_FlightSnapshot_attitude(flight_state), sizeof(*attitude));
	memcpy(attitude_desired, synapse_topic_FlightSnapshot_attitude_desired(flight_state),
...
size_t rdd2_topic_fb_pack_motor_output(uint8_t *buf, size_t buf_size,
				       ...
				       bool test_mode)
{
	...
	length = pack_fixed_table(buf, buf_size, RDD2_TOPIC_FB_MOTOR_OUTPUT_SIZE, MOTOR_VTABLE_SIZE,
				  MOTOR_OBJECT_SIZE, field_offsets, ARRAY_SIZE(field_offsets));
	if (length == 0U) {
		return 0U;
	}
...
bool rdd2_topic_fb_unpack_motor_output(const uint8_t *buf, size_t buf_size,
				       synapse_topic_MotorValues4f_t *motors,
				       synapse_topic_MotorRaw4u16_t *raw, bool *armed,
				       bool *test_mode)
{
	synapse_topic_MotorOutput_table_t output = motor_output_root(buf, buf_size);

	if (output == NULL || motors == NULL || raw == NULL || armed == NULL || test_mode == NULL) {
		return false;
	}

	memcpy(motors, synapse_topic_MotorOutput_motors(output), sizeof(*motors));
	memcpy(raw, synapse_topic_MotorOutput_raw(output), sizeof(*raw));
	*armed = synapse_topic_MotorOutput_armed(output) != 0U;
	*test_mode = synapse_topic_MotorOutput_test_mode(output) != 0U;
