...
static void rdd2_topic_shell_print_latency_stats(const struct shell *sh)
{
	...
	shell_print(sh,
		    "latency us last=%4lu mean=%7.2f std=%7.2f min=%4lu max=%4lu n=%llu",
		    (unsigned long)snapshot.last_us,
		    (double)snapshot.mean_us,
		    (double)snapshot.stddev_us,
		    (unsigned long)snapshot.min_us,
		    (unsigned long)snapshot.max_us,
						 ...
						 const void* msg, size_t msg_size)
{
	...
	shell_print(sh,
		    "status mode=%-10s armed=%1d imu_ok=%1d rc_valid=%1d rc_stale=%1d arm_switch=%1d rc_age_ms=%6lld lq=%3u throttle_us=%4ld imu_to_motor_us=%4lu",
	...
	shell_print(sh,
		    ...
		    (double)accel.z);
	shell_print(sh,
		    "attitude     rpy=" TOPIC_RATE_FMT " attitude_sp  rpy=" TOPIC_RATE_FMT,
	...
	shell_print(sh,
		    "rate_desired xyz=" TOPIC_RATE_FMT " rate_cmd     xyz=" TOPIC_RATE_FMT,
		    ...
		    (double)rate_cmd.yaw);
	shell_print(sh,
		    "rc ch0-7=[%4ld %4ld %4ld %4ld %4ld %4ld %4ld %4ld]",
		    (long)rc.ch0,
		    (long)rc.ch1,
		    (long)rc.ch2,
		    (long)rc.ch3,
						 ...
						 const void* msg, size_t msg_size)
{
	...
	if (!rdd2_topic_fb_unpack_motor_output(msg, msg_size, &motors, &raw, &armed,
					       &test_mode)) {
				       ...
				       const void* msg, size_t msg_size)
{
	...
	shell_print(sh,
		    ...
		    (long)rc->ch7);
	shell_print(sh,
		    "rc ch8-15=[%4ld %4ld %4ld %4ld %4ld %4ld %4ld %4ld]",
		    (long)rc->ch8,
		    (long)rc->ch9,
		    (long)rc->ch10,
		    (long)rc->ch11,
