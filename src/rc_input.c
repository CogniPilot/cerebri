...
static cerebri_topic_RcChannels16_t g_rc_staging;
static cerebri_topic_RcChannels16_t g_rc_latest;
...
static void rc_channels_set_defaults(cerebri_topic_RcChannels16_t *rc, bool *valid, int64_t *stamp_ms)
{
	int32_t *channels = cerebri_topic_rc_channels_data(rc);
...
void cerebri_rc_input_init(void)
{
...
void cerebri_rc_input_latest_get(cerebri_topic_RcChannels16_t *rc, int64_t *stamp_ms,
				  bool *valid)
{
...
uint8_t cerebri_rc_input_link_quality_get(const struct device *dev)
{
...
{
	int32_t *staging_channels = cerebri_topic_rc_channels_data(&g_rc_staging);
	...
	} else if (evt->type == INPUT_EV_MSC && evt->code == CEREBRI_RC_INPUT_EVENT_LINK_QUALITY) {
		atomic_set(&g_rc_link_quality, evt->value);
	} else if (evt->type == INPUT_EV_MSC && evt->code == CEREBRI_RC_INPUT_EVENT_VALID) {
