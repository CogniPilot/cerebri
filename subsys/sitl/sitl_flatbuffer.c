...
#define CEREBRI_SITL_FB_SIM_INPUT_IDENTIFIER "C2SI"
	...
	uint8_t *rc_link_quality, bool *rc_valid, bool *imu_valid)
{
	...
	if (memcmp(buf + 4, CEREBRI_SITL_FB_SIM_INPUT_IDENTIFIER, 4U) != 0) {
		return false;
	}
	...
	if (!sim_input_read_field_offset(table, table_offset, buf_size, SIM_FIELD_GYRO,
	    ...
	    !sim_input_struct_in_bounds(table_offset, object_size, field_offset,
					sizeof(cerebri_topic_Vec3f_t), buf_size)) {
		return false;
	}
	if (gyro != NULL) {
	...
	if (!sim_input_read_field_offset(table, table_offset, buf_size, SIM_FIELD_RC,
					 &field_offset, &object_size) ||
	    !sim_input_struct_in_bounds(table_offset, object_size, field_offset,
					sizeof(cerebri_topic_RcChannels16_t), buf_size)) {
		return false;
	}
	if (rc != NULL) {
